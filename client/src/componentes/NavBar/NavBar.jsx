  ...
  return (
    <nav className={styles.Nav}>
      <div className={styles.divSearch}>
        ...
        {dog && dog.length > 0 && (
          <SearchResultsList setDog={setDog} dog={dog} />
        )}
      </div>
      ...
      <div className={styles.divRefresh}>
       <button onClick={handlerRefresh} className={styles.btnRefresh}>Refresh 🔄</button>
      </div>
      </Link>
      ...
      <div className={styles.logOut}>
        <Link to="/login">
          <button className={styles.btnLogOut} onClick={() => handlerLogOut()}>LogOut↩️</button>
        </Link>
      </div>
    </nav>
  );
